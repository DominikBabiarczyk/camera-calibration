...
PATIENT="${4:-10}"
MIN_DELTA="${5:-0.0}"
TRAIN_SEQUENCES="${6:-100000}"
CALIBRATION_RESULT="${7:-extern/XHOG-007_charuco/result.npz}"
VAL_SEQUENCES="${8:-2000}"
SEED="${9:-42}"

echo "Training fisheye corner LSTM from $CALIBRATION_RESULT"
env PYTHONPATH="$ROOT_DIR" .venv/bin/python -m src.networks_tool.train \
  ...
  --model-name fisheye_corner_lstm_sequence \
  --output-dir outputs/calibration_net_fisheye_lstm \
  --checkpoint-path outputs/calibration_net_fisheye_lstm/best_model.pth \
  --save-epoch-data \
  --epoch-data-dir outputs/calibration_net_fisheye_lstm/epochs \
